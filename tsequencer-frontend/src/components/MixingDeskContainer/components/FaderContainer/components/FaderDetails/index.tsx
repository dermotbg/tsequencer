...
const FaderDetails = ({ instrument, gain }: { instrument: FaderInstrumentTypes; gain: number }) => {
  return (
    <div className="flex flex-col overflow-hidden">
      <div className="sm:max-w-inherit max-w-10 overflow-hidden border-b-2 font-mono font-semibold">
        {instrument === "Pad"
          ? "PAD"
          : `${instrument.charAt(0).toUpperCase()}` +
            `${instrument.charAt(instrument.length - 1).toUpperCase()}`}
      </div>
      <div className="flex flex-col items-center pt-1 font-mono font-semibold">
        <Volume2 size={"16px"} />
